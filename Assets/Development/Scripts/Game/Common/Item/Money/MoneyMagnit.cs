...
public class MoneyMagnit : MonoBehaviour
{
    ...
    public void AttractLinear(Money money)
    {
        ...
        money.transform.DOLocalRotate(Vector2.zero, 0.05f);
    ...
    private IEnumerator Animate(Money money)
    {
        ...
        money.transform.DOLocalRotate(Vector2.zero, _attractDuration);

        while (Vector2.SqrMagnitude(transform.position - money.transform.position) > _followRange)
        {
            float clampedSpeed = Mathf.Clamp(_speed * Time.deltaTime, 0, 1);
            money.transform.position = Vector2.Lerp(money.transform.position, transform.position, clampedSpeed);
        ...
        while (money)
        {
            money.transform.position = Vector2.MoveTowards(money.transform.position, transform.position,
